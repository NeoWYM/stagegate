...
- **What it does**: forces vague intent into executable requirements. Produce a draft with `status: needs_input` + `open_questions`, put them to the user, loop until cleared.
  - **Acceptance-criteria falsifiability review (conditional, mandatory)**: whenever any `acceptance_criteria[]` entry carries a statistical threshold (win rate, coverage, "ratio ≥ X", "CI lower bound > 0"), delegate a **fresh strong-model reviewer (read-only, high effort)** before sign-off to review *only* the acceptance criteria, with exactly two questions: (a) **under what conditions does each criterion fail?** — if no failing scenario can be named, the criterion is vacuous: rewrite or drop it; (b) **given the expected sample size and a plausible effect size, how wide will the confidence interval be, and can the threshold be cleared at all?** — if it cannot, reclassify the lap as *descriptive exploration* and remove the go/no-go from `exit_decision`. Record the verdict in `content.ac_review`.
- **Exit gate**: `scope`/`non_goals`/`constraints`/`acceptance_criteria` complete, no blocking open questions, **`ac_review` completed with no unresolved vacuous / underpowered verdicts when statistical thresholds are present**, explicit user sign-off → `approved` + frozen.
- **Required `content` fields**: `goal` / `scope` / `non_goals` / `constraints` / `acceptance_criteria[]`; conditionally `ac_review` (reviewer, per-criterion failing scenario, power estimate, verdict).

> Why this lives in stage 1 and not only as a second opinion at stage 6: in a 2026-09 backtesting lap, all three acceptance criteria were found defective only at retrospective — an ablation table whose columns were structurally identical by construction, a coverage ratio whose denominator the state machine could never populate (so it read 100% regardless of behaviour), and a "+5pp with CI lower bound > 0" gate that, at n=25 events, had a bootstrap CI ±20pp wide and could not pass under any outcome. All three were computable from the two questions above before a single task ran; instead the lap spent 24 execution tasks to discover them.
...
- **What it does**: decomposes tasks, sets dependencies, orders work. If planning uncovers remaining ambiguity in the requirements → open_question bounce-up; **never decide alone**.
  ...
  - **Size ceiling (budget discipline)**: target **≤ 8 tasks** per lap — beyond that, split the lap (`spawn`, or `continue` into a next lap); keep `03-plan.yaml` **≤ 15 KB** (the orchestrator re-reads it at every stage; a 75 KB plan was one of the largest fixed token costs in the lap cited above). Oversize is a planning defect, not something execution should absorb with more workers.
...
- **What it does**: executes the plan, recording each task's actual result and deviations. **Destructive operations still route through the orchestrator for confirmation** — such steps were marked `human_gate` at planning time.
  ...
  - **Output discipline (budget)**: put output caps in every worker brief — code docstrings **≤ 15 lines**; design decisions and delivery narrative go into `04-execution.yaml` (`results[].deviation` / notes), never into source files; long narrative is written to a file and its path returned. Output tokens cost several times input tokens; a single module docstring exceeding 100 lines is the anti-pattern.
...
6. **Conditional stage (4.5)**: runs only when this lap's artifact is code; non-code or fast-path skips it without blocking. The reviewer must be independent of the executor and run on the strong model. When execution ran in several batches, **consolidate into one review pass** whose brief names the high-risk regions first (look-ahead / data availability / state transitions / destructive paths) — don't open a review per batch.
...
14. **Acceptance-criteria falsifiability (1, conditional)**: `acceptance_criteria` carrying statistical thresholds may not be `approved` without an `ac_review` (fresh strong-model reviewer; two questions: when does it fail, and can the threshold be cleared at the expected sample size). Verdict *vacuous* → rewrite or drop the criterion; verdict *underpowered* → either the lap's `exit_decision` becomes descriptive (no go/no-go) or the threshold is changed — one or the other. **Never carry an unpassable gate into stage 3.**
...
- **Session splitting (budget)**: cut sessions at stage boundaries — stages 1–3 in one session (they need the user anyway), stage 4 in another (long compute waits such as backtests or deploys burn no tokens), stages 5–6 in a third. Each session cold-starts from the frozen documents with no carried-over conversation. The frozen handoffs exist precisely to make this cheap.
- **Budget discipline**: the foreground orchestrator defaults to the **fast tier at high effort**; escalating the orchestrator itself to a premium tier is a manual user decision, never something the pipeline does on its own. Read-back verification goes to the cheapest tier. Strong-model second opinions (stage-1 `ac_review`, stage-6 exit) are cheap — one call carrying a ≤ 5 KB evidence document — and are **not** where to save; save on stage-4 task count, plan-document size, and worker output length.
