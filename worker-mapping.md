...
|-------|--------|-----------|-------|
| 1 requirements (conditional `ac_review`) | a read-only reviewer agent (tool-level read-only boundary) | **strong, high effort** | Fires only when `acceptance_criteria` carries statistical thresholds. The brief pastes the criteria list, the expected number of signals/samples, and the population size, and asks exactly two questions: under what conditions does each criterion fail; how wide is the CI at that sample size and can the threshold be cleared. Report ≤ 10 lines, written into `01.content.ac_review`. No shell access is deliberate — it reads a requirements draft, it doesn't query systems |
